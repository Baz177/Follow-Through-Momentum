...
def process_data():
    try:
        ...
        assert len(tickers) == len(names), "Ticker and Company lists have different lengths"

        # Zip and take first 100 pairs
        ticker_names = zip(tickers, names)
        #positive_df = stock_with_positive_change(list(islice(ticker_names, 100)))
        positive_df = stock_with_positive_change(ticker_names)
